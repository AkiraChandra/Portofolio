...
const transformDatabaseCertification = async (
  dbCertification: CertificationDB
  ...
  try {
    ...
    const expirationDate = dbCertification.expiration_date
      ? new Date(dbCertification.expiration_date)
      : undefined;
    ...
    try {
      ...
      if (options.filters) {
        ...
        if (issueDateRange?.start) {
          query = query.gte(
            "issue_date",
            issueDateRange.start.toISOString().split("T")[0]
          );
        }
        ...
        if (issueDateRange?.end) {
          query = query.lte(
            "issue_date",
            issueDateRange.end.toISOString().split("T")[0]
          );
        }
        ...
        if (expirationDateRange?.start) {
          query = query.gte(
            "expiration_date",
            expirationDateRange.start.toISOString().split("T")[0]
          );
        }
        ...
        if (expirationDateRange?.end) {
          query = query.lte(
            "expiration_date",
            expirationDateRange.end.toISOString().split("T")[0]
          );
        }
      }
      ...
      if (options.query) {
        query = query.or(
          `name.ilike.%${options.query}%,issuing_organization.ilike.%${options.query}%,description.ilike.%${options.query}%`
        );
      }
      ...
      return await Promise.all(
        certifications.map((certification) =>
          transformDatabaseCertification(certification)
        )
      );
    } catch (error) {
