...
const FeaturedCertifications = memo(
  ({ certifications, onCertificationClick }: FeaturedCertificationsProps) => {
    ...
    return (
      <section className="mb-4 mb:mb-10">
        ...
        <div className="relative px-0 md:px-0">
          {/* Card Display System */}
          <div className="flex justify-center items-center py-4 overflow-hidden">
            <div className="relative w-full max-w-6xl">
              <div className="flex items-center justify-center min-h-[160px] overflow-hidden">
                {certifications.map((cert, index) => {
                  ...
                  const isVisible =
                    Math.abs(position) <=
                    Math.floor(visibleConfiguration.visible / 2);
        ...
        <div className="flex justify-center items-center gap-2 mt-2">
          ...
          <div className="flex items-center gap-2 text-xs text-text-tertiary dark:text-text-tertiary-dark">
            <div
              className={`w-2 h-2 rounded-full bg-green-400 transition-colors duration-300`}
            />
            <span>Auto-rotating</span>
          </div>
          {/* Navigation Control - Kanan */}
          <motion.button
            onClick={handleNext}
            className="p-2 md:p-3 rounded-full bg-background-secondary/80 dark:bg-background-secondary-dark/80 backdrop-blur-sm border border-border-primary/30 dark:border-border-primary-dark/30 text-text-primary dark:text-text-primary-dark hover:bg-primary/10 dark:hover:bg-primary-dark/10 transition-all duration-300"
            whileHover={{ scale: 1.1 }}
  ...
  ({
    ...
    return (
      <motion.div
        ...
        style={{
          width: isMobile ? "280px" : isTablet ? "500px" : "600px",
          height: "160px",
          left: "50%",
        ...
        whileHover={
          isCenter
            ? { y: -4, scale: 1.02 }
            : { scale: transformStyles.scale * 1.05 }
        }
      >
      ...
      `}
        >
          ...
          <div className="flex gap-3 h-full p-3">
            ...
            <div
              className="h-full flex-shrink-0 mt-1"
              style={{
              ...
              <div className="w-full h-full rounded-lg overflow-hidden bg-background-tertiary/30 dark:bg-background-tertiary-dark/30 border border-border-primary/10 dark:border-border-primary-dark/10">
                {certification.certificateImage && !imageError ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Award className="w-20 h-20 text-text-tertiary dark:text-text-tertiary-dark" />
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              <div className="mb-1.5">
                <h3
                  className={`font-bold text-text-primary dark:text-text-primary-dark leading-tight transition-colors duration-300 ${
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  <Building2
                    ...
                    } text-text-tertiary dark:text-text-tertiary-dark flex-shrink-0`}
                  />
                  <span
                    className={`${
                      isMobile ? "text-[10px]" : "text-xs"
                    } text-text-secondary dark:text-text-secondary-dark truncate`}
                  >
                ...
                <div className="flex items-center gap-1 text-text-tertiary dark:text-text-tertiary-dark">
                  ...
                  <span className="text-[10px] font-medium">
                    ...
                    {certification.expirationDate && (
                      <span>
                        {" "}
                        - {certification.expirationDate.getFullYear()}
                      </span>
                    )}
                  </span>
                </div>
